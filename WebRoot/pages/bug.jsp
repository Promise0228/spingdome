...
<body>
	...
	<div class="container-fluid">
		...
		<!-- 自动补全div -->
		<div id="word" style="position: absolute;top: 170px;left:243px;"></div>
		<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
			<h1 class="page-header">BUG列表</h1>
			<div class="row placeholders">
				<div>
					<form action="${pageContext.request.contextPath}/bug/bugList.action" method="post">
						<input type="text" name="bugTitle" id="bugTitle" value="${listss.bugTitle }" style="height: 30px; width: 150px"
							 placeholder="用户名">
							  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
						<input type="text" name="updateBys" id="updateBys" value="${listss.updateBys }" style="height: 30px; width: 150px" 
						placeholder="执行人">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
						 <input type="date" name="createTime" id="createTime" value="${listss.createTime }" pattern='yyyy-MM-dd' />
						&nbsp;&nbsp;&nbsp;-&nbsp;&nbsp;&nbsp; 
						<input type="date" name="createTime1" id="createTime1" value="${listss.createTime1 }" pattern='yyyy-MM-dd' />
						<br><br>
				...
				<div>
					...
					<c:if test="${fn:contains(authcodes,'add_bug')}">
						<button type="button" class="btn btn-primary show-user-form"
							data-toggle="modal" data-target="#add-user-form">添加新BUG</button>
					</c:if>
						...
						<div class="modal-dialog modal-sm" role="document">
							<div class="modal-content">
								<div class="modal-header">
									...
									<h4 class="modal-title">添加新BUG</h4>
								</div>
								<div class="modal-body">
									<form class="user-form">
										...
										<div class="form-group">
											<label for="projName">所属项目</label> 
											<select id="projName1" onclick="findClassByProj1()">
								...
								<div class="modal-footer">
									<button type="button" class="btn btn-default"
										data-dismiss="modal">取消</button>
									<button type="button" class="btn btn-primary add-bug">添加
									</button>
								</div>
							</div>
						</div>
					...
					<c:forEach items="${bugList }" var="bug">
						<tr>
							...
							<td><c:if test="${bug.bugState!=3}">
									...
									<c:if test="${fn:contains(authcodes,'update_bug')}">
								...
								<c:if test="${fn:contains(authcodes,'fenpei_bug')}">
								
								<button type="hidden" ${(user.userState=='0')?'style="display:none"':''} class="btn btn-primary fenpeibug"  data-toggle="modal" id="assignRoles" data-target="#assign-roles" value="${bug.projId}">分配BUG</button>
								
								</c:if>
								<c:if test="${fn:contains(authcodes,'xiangqing_bug')}">
								...
								<c:if test="${fn:contains(authcodes,'delete_bug')}">
								<c:if test="${bug.bugState==3 }">
										<a class="glyphicon glyphicon-remove delete-this-user"
											aria-hidden="true" title="删除BUG"  }"></a>
									</c:if>
									</c:if>
								</td>
						</tr>
...
<div class="modal fade " id="update-userrole-dialog" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel">
	...
	<div class="modal-content">
		...
		&nbsp;&nbsp;&nbsp;&nbsp;
        &nbsp;&nbsp; 选择文件:<input type="file" name="file" accept="image/*" onsubmit="checkImgType()">
        <input type="file" name="file1" accept="image/*"  onblur="checkImgType()">
        <input type="file" name="file2" accept="image/*" onsubmit="checkImgType()">
					<div class="modal-footer">
					<button type="button" class="btn btn-default"
					data-dismiss="modal">取消</button>
...
<!-- END DOM -->
 ...
 var flag=false;
	...
	$("#userNameInput").blur(function (){
		...
		var classId=$("#classId1").val();//获取bug所属模块
		var projId=$("#projName1").val();//获取bug所属项目
		...
		$(".user-list").on("click", ".delete-this-user", function() {
			...
			var bugState=userTr.find(".bugState").html();
			...
			var projNum =$("#projNums").val();
				...
				$.ajax({
					...
					var res = $.parseJSON(data);//把后台传回的json数据解析
					 var option;
					 if(res==null||res==""){
					 option = "<option value='"+0+"'>"+"该项目下无模块"+"</option>"
...
//---------------------------------自动补全--------------------------------
        $(function(){
//当键盘键被松开时发送Ajax获取数据
		$('#bugTitle').keyup(function(){
			var keywords = $(this).val();
			if (keywords=='') { $('#word').hide(); return };
			$.ajax({
				url:"${pageContext.request.contextPath}/bug/auto-completion.action",
				data:{
				bugTitle:keywords
				},
				dataType: 'jsonp',
